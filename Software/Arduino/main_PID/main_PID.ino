...
void loop() 
{
  ...
  if (i != old_i)
  {
    old_i = i;
    Serial.println(i);
    int motor_value = computePID(current_heading, 0);
    xmotor.set_motor_dir_speed(1, motor_value);
  }
  ...
  char buffer2[200];
  sprintf(buffer2, "BatteryData:%f:%f:%f:%f:%f", ssensor_data.cell1v, ssensor_data.cell2v, ssensor_data.cell3v, ssensor_data.tempp, i);
  sendMessage(buffer2);
